...
elasDir = 'npy-color-augmented-elastic'
tol = 4
epochs = 100
try:
...
def fetch_data(X, Y):
    ...
    while True:
        yield X[index:index+BATCH_SIZE].compute().astype('float32') / 255, \
              Y[index:index+BATCH_SIZE].compute().astype('float32')
        
        index = (index + BATCH_SIZE) % L
...
if grayscale:
    xshape = (None, NUM_CONSEC, HEIGHT, WIDTH)
else:
    xshape = (None, NUM_CONSEC, HEIGHT, WIDTH, 3)
yshape = (None, NUM_CONSEC, HEIGHT, WIDTH)

data_generator = fetch_data(x_data, y_data)
dataset = tf.data.Dataset.from_generator(
    lambda: data_generator,
    output_shapes=(xshape, yshape),
    output_types=(tf.float32, tf.float32),
).shuffle(buffer_size=200).prefetch(tf.data.experimental.AUTOTUNE)
